...
// QueryScalar runs an instant query and returns the first result series' value,
// ignoring labels. ok is false when the query returns no (finite) sample — e.g.
// an aggregation like min(up{job="x"}) over a target that isn't scraped. Used by
// the status page for `up` / `avg_over_time(up[…])` probes.
func (c *Client) QueryScalar(ctx context.Context, query string) (value float64, ok bool, err error) {
	u := c.baseURL + "/api/v1/query?" + url.Values{"query": {query}}.Encode()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, u, nil)
	if err != nil {
	...
	if pr.Status != "success" {
		return 0, false, fmt.Errorf("prometheus query failed: %s: %s", pr.ErrorType, pr.Error)
	}
	for _, series := range pr.Data.Result {
		...
		v, perr := strconv.ParseFloat(sample, 64)
