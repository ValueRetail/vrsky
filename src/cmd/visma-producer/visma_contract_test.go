...
func TestContract_VismaProducerSendsWhatWasPublished(t *testing.T) {
	for _, ce := range contract.Load(t, "visma") {
		t.Run(ce.Mode, func(t *testing.T) {
			tokenSrv := tokenServer()
			...
			if gotAuth == "" {
				t.Error("no Authorization header reached Visma")
			}
