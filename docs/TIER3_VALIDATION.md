...
- **Autoscaling (#19):** drive the tenant past the **connection-count** trigger (≥50 connections). The msg/s trigger cannot fire and must not be used as the test: a placed connection's traffic never reaches the instance it is placed on (#209), so the scraped rate is always 0. See the warning at the top of [NATS_ARCHITECTURE.md](NATS_ARCHITECTURE.md). **Pass:** the autoscaler provisions instance #2 (`kubectl get pods -n vrsky-tenants`), `vrsky_nats_instance_capacity_pct` crosses 80 (the `NATSInstanceApproachingCapacity` alert fires), and new connections place onto the new instance (`SELECT nats_instance_id, count(*) FROM connections GROUP BY 1`). Drain a tenant's connections off an extra instance → it's decommissioned.
